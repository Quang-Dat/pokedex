// src/app/pokemon/[id]/edit/page.jsx
"use client";
...
export default function EditPokemon() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Odeslání dat na API endpoint pro aktualizaci
      const res = await fetch(`/api/pokemon/${id}/update`, {
        method: "PUT",
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.message || "Nepodařilo se aktualizovat Pokémona"
        );
      }

      setSuccess(true);
      // Přesměrování zpět na detail po úspěšné aktualizaci
      setTimeout(() => {
  ...
  if (error && !pokemon) return <div>Chyba: {error}</div>;
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      >
        &larr; Zpět na detail Pokémona
      </Link>

      <h1 className="text-3xl font-bold mb-6">Upravit Pokémona #{id}</h1>
      ...
      {success && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
          Pokémon byl úspěšně aktualizován! Přesměrování...
        </div>
      )}
      ...
      >
        ...
        <div className="mt-8 flex justify-end">
          <button
            type="submit"
            className="bg-yellow-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-yellow-700 disabled:opacity-50"
            disabled={submitting}
          >
            {submitting ? "Ukládání..." : "Aktualizovat Pokémona"}
          </button>
        </div>
      </form>
    </div>
